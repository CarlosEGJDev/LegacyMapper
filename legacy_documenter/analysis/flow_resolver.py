...
class FunctionalFlowResolver:
    ...
    def resolve(
        ...
        self._path_identities: dict[str, str] = {}
        ...
        for entry in entry_points:
            ...
            paths = sorted(path_bucket.values(), key=lambda item: item["path_id"])
        ...
        path_list = sorted(all_paths.values(), key=lambda item: (item["flow_id"], item["path_id"]))
        summary = self._summary(entry_points, flows, path_list, errors or [])
        return flows, path_list, summary, unresolved

    def _walk(self, entry, method_key, node_path, relation_types, edge_refs, graph_nodes, graph_edges, paths) -> None:
        method_label = self._method_label(method_key)
        self._add_node(graph_nodes, self._node("Method", method_label, method_label, method_key[2]))
        self._add_node(graph_nodes, self._node("Class", self._class_id(method_key), method_key[0], method_key[2]))
        if method_key[2]:
            self._add_node(graph_nodes, self._node("Project", method_key[2], method_key[2]))
        if len(node_path) > self.max_depth:
            self._add_path(paths, entry, node_path, relation_types, "truncated_depth", method_label, "confirmed", edge_refs)
            return
        ...
        for operation in data_ops:
            ...
            if terminal_nodes:
                for terminal_type, terminal_id, terminal_target, confidence in terminal_nodes:
                    node_type = "StoredProcedure" if terminal_type == "stored_procedure" else "SQL"
                    self._add_node(graph_nodes, self._node(node_type, terminal_id, terminal_target))
                    ...
                    self._add_path(paths, entry, node_path + [op_id, terminal_id], relation_types + ["Method -> DataAccessOperation", edge_type], terminal_type, terminal_target, self._weakest([operation.get("confidence"), confidence]), edge_refs + [op_id])
            else:
                self._add_path(paths, entry, node_path + [op_id], relation_types + ["Method -> DataAccessOperation"], "data_operation", op_id, operation.get("confidence", "confirmed"), edge_refs + [op_id])

        followed = False
        calls = self.method_calls.get(method_key) or self.method_calls.get((method_key[0], method_key[1], None), [])
        for call in calls:
            ...
            if target_label in node_path:
                self._add_path(paths, entry, node_path + [target_label], relation_types + ["Method -> Method"], "cycle", target_label, "confirmed", edge_refs + [call_ref])
                followed = True
                continue
            followed = True
            self._walk(entry, target_key, node_path + [target_label], relation_types + ["Method -> Method"], edge_refs + [call_ref], graph_nodes, graph_edges, paths)

        unresolved_calls = self.unresolved_calls.get(method_key) or self.unresolved_calls.get((method_key[0], method_key[1], None), [])
        for call in unresolved_calls:
            ...
            self._add_path(paths, entry, node_path + [boundary_id], relation_types + ["Method -> UnresolvedCall"], "unresolved_boundary", call.get("expression") or boundary_id, "unresolved", edge_refs + [self._call_ref(call)])
            followed = True

        if not data_ops and not followed:
            self._add_path(paths, entry, node_path, relation_types, "dead_end", method_label, "confirmed", edge_refs)
    ...
    def _index_calls(self, calls: list[dict]) -> tuple[dict[tuple, list[dict]], dict[tuple, list[dict]]]:
        ...
        for bucket in (confirmed, unresolved):
            for key in bucket:
                bucket[key].sort(key=lambda item: self._call_ref(item))
    ...
    def _index_data_access(self, data_access: list[dict]) -> dict[tuple, list[dict]]:
        ...
        for key in result:
            result[key].sort(key=lambda item: item.get("id") or "")
        return result
    ...
    def _terminal_nodes(self, operation: dict) -> list[tuple[str, str, str, str]]:
        result = []
        for proc in self.proc_by_operation.get(operation.get("id"), []):
            result.append(("stored_procedure", proc.get("id"), proc.get("name") or proc.get("id"), proc.get("confidence", "confirmed")))
        for sql in self.sql_by_operation.get(operation.get("id"), []):
            result.append(("sql", sql.get("id"), sql.get("operation") or sql.get("id"), sql.get("confidence", "confirmed")))
        if not result and operation.get("stored_procedure"):
            result.append(("stored_procedure", operation["stored_procedure"], operation["stored_procedure"], operation.get("confidence", "confirmed")))
        if not result and operation.get("sql_operation"):
            result.append(("sql", operation.get("id"), operation.get("sql_operation"), operation.get("confidence", "confirmed")))
        return result
    ...
    def _add_path(self, paths: dict[tuple, dict], entry: dict, nodes: list[str], relation_types: list[str], terminal_type: str, terminal_target: str, confidence: str, evidence_refs: list[str]) -> None:
        ...
        if key in paths:
            return
        path_id, canonical_identity = self._path_id(entry.get("id"), nodes, relation_types, terminal_type, terminal_target)
        existing_identity = self._path_identities.get(path_id)
        if existing_identity is not None and existing_identity != canonical_identity:
            raise ValueError(f"Functional path ID collision: {path_id}")
        self._path_identities[path_id] = canonical_identity
        paths[key] = {
            "flow_id": None,
            "path_id": path_id,
            "entry_point_id": entry.get("id"),
    ...
    def _path_id(self, entry_point_id: str | None, nodes: list[str], relation_types: list[str], terminal_type: str, terminal_target: str) -> tuple[str, str]:
        identity = {
        ...
        return f"PATH-{hashlib.sha256(canonical.encode('utf-8')).hexdigest()}", canonical
