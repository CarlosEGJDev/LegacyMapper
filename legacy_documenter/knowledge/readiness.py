...
Parsing (of the human-review record and Markdown documents), evidence-closure
computation, and file-I/O primitives were extracted into the internal
`legacy_documenter.knowledge._readiness_parsing`, `_readiness_evidence`, and
`_readiness_io` modules under V4.1-R4 (DEBT-002). This module remains the
sole public entry point and compatibility facade: every name previously
importable from `legacy_documenter.knowledge.readiness` -- including `run()`
and the `python -m legacy_documenter.knowledge.readiness` CLI behavior --
still resolves here unchanged.
"""
import json
import re
...
from legacy_documenter.knowledge._readiness_evidence import _canonical_catalog, evidence_closed, evidence_closure_diagnostics
from legacy_documenter.knowledge._readiness_io import _hash, _read
from legacy_documenter.knowledge._readiness_parsing import EXPECTED, HUMAN_CONFIRMED, _csv, _details, _field, parse_human_record
