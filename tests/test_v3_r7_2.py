...
# output/v2_r5_1_full/ is the full V2-R5.1 real-legacy-repository scan dump. It is
# intentionally untracked/.gitignore'd (see docs/GENERATED_ARTIFACT_POLICY.md) and can
# only be regenerated by scanning the real legacy source repository, which this test
# suite must not do (POST_V4_2_FRESH_CLONE_REPRODUCIBILITY_CORRECTION). On a fresh
# clone this fixture is absent by design; skip rather than error or fabricate it.
_V2_R5_1_FULL = Path(__file__).parents[1]/"output"/"v2_r5_1_full"/"ai_context"/"SYSTEM_CONTEXT.json"
